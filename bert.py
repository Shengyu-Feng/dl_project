...
# Modified from Bert examples to arxivv paper summaries
...
def convert_examples_to_features(examples, seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        tokens_og = tokenizer.tokenize(example.text)
        ...
        if len(tokens_og) > seq_length - 2:
            tokens_og = tokens_og[0:(seq_length - 2)]
...
def main():
    ...
    eval_data = TensorDataset(
        all_input_ids, all_input_mask, all_example_index)
    if local_rank == -1:
